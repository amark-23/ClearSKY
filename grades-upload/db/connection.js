...
const db = new sqlite3.Database(path.join(__dirname, 'grades_upload.sqlite'), (err) => {
    if (err) console.error('❌ Error opening database:', err);
    else console.log('✅ Connected to SQLite grades_upload.sqlite database');
});
...
`, (err) => {
    if (err) {
        console.error('[GRADES-UPLOAD] ❌ Error creating subjects table:', err.message);
    } else {
        // Mock subjects με έναν καθηγητή το καθένα
        ...
        db.run(`INSERT OR IGNORE INTO subjects (subjectID, subjectName, instructorID) VALUES (303, 'Μαθηματικά', 3)`, () => {
            console.log('[GRADES-UPLOAD] ✅ Subjects initialized');
        });
    }
});
